...
for i_day in range(0,31):
    ...
    if opt_linear == 0:
        ...
        m_state_dict_model = torch.load('Results/'+str(nn_zone)+'_full_Model_ind_Y_lin.pt')
    M_model_deter = MZ_Model_Func.model_joint(ae_input_num,ae_hidden_units,ae_output_num,mm_hidden_units,opt_linear,'False','False',[m_state_dict_Y,m_state_dict_P,m_state_dict_X],m_state_dict_model).to(device)
    if opt_linear == 0:
        state_dict_joint = torch.load('Results/'+str(nn_zone)+'_full_Model_joint.pt')
    else:
        state_dict_joint = torch.load('Results/'+str(nn_zone)+'_full_Model_joint_lin.pt')
    ...
    for i_noise in range(0,11):  #11
        if i_noise > 0 and i_day not in [0,10,20]:
            ...
            M_obj = MZ_Model_Func.NN_obj_layer(ae_input_num,ae_hidden_units,ae_output_num,mm_hidden_units,[m_state_dict_Y,m_state_dict_P,m_state_dict_X],m_state_dict_model,state_dict_joint,ten_Y_max,ten_Y_min,ten_P_max,ten_P_min,wei_Y,wei_P,opt_linear)
            ...
            for iter in range(0, Iters3):
                if iter == 0:
                    ...
                    Iter_J_tr_en[iter,:,ae_output_num[0]:sum(ae_output_num[:2])] = Iter_J_tr_en[iter-1,:,ae_output_num[0]:sum(ae_output_num[:2])] - Iter_lr*Iter_sens_grd[iter-1,:,:]
                ...
                time1 = time.time()
                Obj_Y_upp,Obj_Y_low,Obj_P_1,Obj_P_2upp,Obj_P_2low,_,_ = \
                ...
                Iter_sens_grd[iter,:,:] = J_tr_en_P_ini.grad
            ...
            plt.plot(Iter_obj.iloc[:iter+1,-1])
            plt.show()
            ...
            M_nn_input,M_nn_hidden,M_nn_output = M_train_data.shape[-1],[128,64,64],nn_zone
            M_model_NN = MZ_Model_Func.model_FNN(M_nn_input, M_nn_hidden, M_nn_output,0).to(device)
            m_state_dict = torch.load('Results/'+str(nn_zone)+'_whole_NN.pt')
            ...
            Final_true_cost2 = pho_tem*(1/T_Fre)*sum(sum(relu(Final_de_Y_hy[t,i]-c_upper_tem[opt_start+t,i])**2 + relu(c_lower_tem[opt_start+t,i] - Final_de_Y_hy[t,i])**2 for i in range(c_upper_tem.shape[1])) for t in range(opt_period))
            print(Iter_obj.iloc[final_i,0:2])
            print(Final_true_cost1,Final_true_cost2_true,Final_true_cost2)

            if i_noise == 0:
                HZ_AE_P[i_day,:,:] = Final_de_P_hy_jiu[:,:]
                HZ_AE_Tem[i_day,:,:] = Final_true_hy[:,:]
                HZ_AE_Tem_diff[i_day,:,:] = Final_true_hy[:,:]-Final_de_Y_hy[:,:]
            ...
            if i_noise == 0 and i_day == 20:
                np.save('MZ_results/HZ_AE_loss.npy',Iter_obj.iloc[:final_i+1,-1].to_numpy())

np.save('MZ_results/HZ_AE_P.npy',HZ_AE_P)
np.save('MZ_results/HZ_AE_Tem.npy',HZ_AE_Tem)
np.save('MZ_results/HZ_AE_Tem_diff.npy',HZ_AE_Tem_diff)
np.save('MZ_results/HZ_AE_obj.npy',HZ_AE_obj)
np.save('MZ_results/Noise_AE_obj.npy',Noise_AE_obj)
